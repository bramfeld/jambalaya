...
public class JButtonList extends JList implements MouseListener, KeyListener,
	...
	private ArrayList buttons = new ArrayList(0);
	...
	private ArrayList contextMenuActions = new ArrayList(2);
	 ...
	 * Must be a subclass of {@link AbstractButton}.
	 * @param abstractButton
	 * @throws IllegalArgumentException if the class isn't a subclass of {@link AbstractButton}
	 */
	public void setButtonClass(Class abstractButton) throws IllegalArgumentException {
		if (abstractButton == null) {
			abstractButton = JCheckBox.class;
		}
		if (!abstractButton.equals(buttonClass)) {
			try {
				AbstractButton button = (AbstractButton) abstractButton.newInstance();
				button.setSelected(false);	// remove the warning
			} catch (Exception e) {
				throw new IllegalArgumentException("Invalid button class.  Must be a subclass of AbstractButton.");
			}
	...
	public Object[] getCheckedItems() {
		final Collection items = getCheckedItemsCollection();
	 ...
	 */
	public Collection getCheckedItemsCollection() {
	 ...
	 * @param items the items to check
	 */
	public void setCheckedItems(Collection items) {
		for (int i = 0; i < getModel().getSize(); i++) {
			Object obj = getModel().getElementAt(i);
			AbstractButton button = (AbstractButton) buttons.get(i);
	...
	public int[] getCheckedIndeces() {
		ArrayList checkedIndeces = new ArrayList();
		for (int i = 0; i < getModel().getSize(); i++) {
			AbstractButton button = (AbstractButton) buttons.get(i);
			if (button.isSelected()) {
				checkedIndeces.add(new Integer(i));
			}
		}
		int[] indeces = new int[checkedIndeces.size()];
		for (int i = 0; i < indeces.length; i++) {
			indeces[i] = ((Integer)checkedIndeces.get(i)).intValue();
		}
		return indeces;
	}
	...
	public void setCheckedIndices(int[] checkedIndeces) {
		...
		if ((checkedIndeces != null) && (checkedIndeces.length > 0)) {
			for (int i = 0; i < checkedIndeces.length; i++) {
				int index = checkedIndeces[i];
				if ((index >= 0) && (index < buttons.size())) {
					AbstractButton button = (AbstractButton) buttons.get(index);
	...
	public AbstractButton getButton(Object element) {
		...
		for (int i = 0; i < getModel().getSize(); i++) {
			...
			if ((obj != null) && obj.equals(element)) {
				if (i < buttons.size()) {
					button = (AbstractButton) buttons.get(i);
				}
				break;
			}
		}
	...
	public AbstractButton getButton(int index) {
		AbstractButton button = null;
		if ((index >= 0) && (index < buttons.size())) {
			button = (AbstractButton) buttons.get(index);
		}
		return button;
	}
	...
	public void setEnabled(boolean enabled) {
		...
		for (Iterator iter = buttons.iterator(); iter.hasNext(); ) {
			AbstractButton btn = (AbstractButton) iter.next();
		...
		if (e.getIndex1() >= e.getIndex0()) {
			for (int index = e.getIndex1(); index >= e.getIndex0(); index--) {
				if (index < buttons.size()) {
					...
					AbstractButton btn = (AbstractButton) buttons.get(index);
					btn.setText(text);
	...
	public void keyPressed(KeyEvent e) {
		...
		if ((code == KeyEvent.VK_SPACE) || (code == KeyEvent.VK_ENTER)) {
			int[] indeces = getSelectedIndices();
			for (int i = 0; i < indeces.length; i++) {
				toggleCheckBox(indeces[i]);
			}
		}
	}
	...
	public void popupMenuWillBecomeVisible(PopupMenuEvent e) {
		...
		popupMenu.removeAll();
		for (Iterator iter = contextMenuActions.iterator(); iter.hasNext(); ) {
	...
	protected void removeButton(int index) {
		if ((index >= 0) && (index < buttons.size())) {
			AbstractButton button = (AbstractButton) buttons.remove(index);
	...
	protected void fireItemSelected(ItemEvent ie) {
		if (listeners.size() > 0) {
			...
			for (int i = 0; i < clone.size(); i++) {
				((ItemListener) clone.get(i)).itemStateChanged(ie);
			}
		}
	}
	...
	protected void reload(boolean repaint) {
		...
		while (buttons.size() > 0) {
			removeButton((AbstractButton) buttons.get(0));
		}
	...
	private AbstractButton createButton(Object obj) {
		...
		try {
			...
			button = (AbstractButton) buttonClass.newInstance();
		} catch (Exception e) {
	...
	private class ExtendedButtonGroup extends ButtonGroup {
		...
		public void clearButtons() {
			Vector clone = new Vector(buttons);
			for (int i = 0; i < clone.size(); i++) {
				remove((AbstractButton) clone.get(i));
			}
		}
	}
		...
		combo.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED) {
					Class cls = (Class) combo.getSelectedItem();
